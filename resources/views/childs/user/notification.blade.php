...
<section class="content">
    <div>
            ...
            <form method="get" action="/media/notification" autocomplete="off" role="form" class="form-inline">
                ...
                <div class="form-group">
                    <select class="form-control" name="status" data-live-search="true" data-width="150px">
                        ...
                        <option value="Editor" @if(isset($request_all['user_type']) && $request_all['user_type'] =='Active' ){{"selected"}}@endif> Active</option>
                        <option value="Admin" @if(isset($request_all['user_type']) && $request_all['user_type'] =='Inactive' ){{"selected"}}@endif> Inactive</option>
                    </select>
                </div>
                <div class="form-group">
                    <select class="form-control" name="device" data-live-search="true" data-width="150px">
                        <option value="" @if(!isset($request_all['device'])){{"selected"}}@endif >{{ trans('user.device') }}</option>
                        @foreach( $devices as $item )
                        <option value="{{ $item->id }}" @if(isset($request_all['device']) && (int)$request_all['device'] ==$item->id ){{"selected"}}@endif>{{ $item->devicename }} </option>
                        @endforeach
                    </select>
                </div>
        ...
        <div style="width: 50%;padding-bottom: 5px; margin-bottom: 5px; border-bottom: 1px solid #ddd;overflow: hidden;">
            ...
            <div class="form-group">
                <label><i class="fa fa-paragraph"></i> Thiết bị</label>
                <select class="form-control" name="device-push" >
                    <option value="0" @if(!isset($request_all['device'])){{"selected"}}@endif >{{ trans('user.device') }}</option>
                    @foreach( $devices as $item )
                    <option value="{{ $item->id }}" @if(isset($request_all['device']) && (int)$request_all['device'] ==$item->id ){{"selected"}}@endif>{{ $item->devicename }} </option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <button id="btnPushText" class="btn btn-danger" name="push_text" onclick="javascript:pushText()">Push text</button>
            </div>
        </div>
    </div>
</section>
...
//                            + start_date + '&end_date=' + end_date
                        var str_data = 'id=' + id + '&title=' + title + '&content=' + content + '&start_date=' + start_date + '&device=' + device;
                        $.ajax({
                            type: 'POST',
                            url: api,
                            data: str_data,
