...
@section('main_content')
    <section class="content-header">
        <h1>{{trans('category.edit_title')}}</h1>
        <ol class="breadcrumb">
            <li><a href="javascript:void(0)"><i class="fa fa-dashboard"></i> Home</a></li>
            <li class="active">{{trans('menu.edit_title')}}</li>
        </ol>
    </section>
    <!-- Main content -->
                        ...
                        <div class="box-body">
                            ...
                            <div class="form-group">
                                ...
                                <select name="parent_id" class="form-control" style="width: 100%;" tabindex="-1" aria-hidden="true">
                                    ...
                                    <?php cate_parent($parentCats, 0, "--",$category['parent_id']); ?>
                                </select>
                            </div>

                            <div class="form-group">
                                <label>Mô tả</label>
                                <textarea name="description" rows="3" class="form-control">{!! $category['desc'] !!}</textarea>
                            </div>

                            <div class="form-group">
                                <label>{{trans('category.status')}}</label>
                                <select name="status" class="form-control" style="width: 100%;" tabindex="-1" aria-hidden="true">
                                    <option value="0">{{trans('category.inactive')}}</option>
                                    <option value="1" {{($category['status'] == 1) ? 'selected="selected"' : ''}}>{{trans('category.active')}}</option>
                                </select>
                            </div>
                        </div>
                        <!-- /.box-body -->
